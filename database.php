...
function joinTableWherein($table, $joins, $conditions = [], $groupBy = '')
{
    global $conn;
    $query = "SELECT * FROM $table";
    foreach ($joins as $join) {
...
function find_where($table, $columns_and_values, $limit = null, $order_by = null)
{
    global $conn;
    $sql = "SELECT * FROM $table WHERE ";
    $params = [];
    ...
    if (!empty($nullables)) {
        $nullablesSql = implode(" IS NULL AND ", array_keys($nullables));
        $sql .= "AND $nullablesSql IS NULL ";
    }
...
function countResult($table, $conditions)
{
    global $conn;
